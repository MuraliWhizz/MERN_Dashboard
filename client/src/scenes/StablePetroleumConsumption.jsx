...
const StablePetroleumConsumption = () => {
  const { data, isLoading } = useGetEnergyInsightsQuery({ page: 1, pageSize: 1000, sector: 'Energy', topic: 'consumption' });

  if (isLoading) {
    return <div>Loading...</div>;
  }

  const filteredData = data.filter(item => item.title.includes('U.S. petroleum consumption is projected to remain below the 2005 level'));
  ...
  return (
    <div style={{ margin: '40px' }}>
      <h2>Stable Petroleum Consumption</h2><br></br>
...
export default StablePetroleumConsumption;