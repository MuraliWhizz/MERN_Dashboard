...
const RecoveryOfCrudeOilProduction = () => {
  const { data, isLoading } = useGetEnergyInsightsQuery({ page: 1, pageSize: 1000, sector: 'Energy', topic: 'oil' });

  if (isLoading) {
    return <div>Loading...</div>;
  }

  const filteredData = data.filter(item => item.title.includes('Reference case U.S. crude oil production is projected to recover'));
  ...
  return (
    <div style={{ margin: '40px' }}>
      <h2>Recovery of Crude Oil Production</h2><br></br>
...
export default RecoveryOfCrudeOilProduction;