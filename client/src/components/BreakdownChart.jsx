...
function BreakdownChart({ isDashboard = false }) {
  ...
  const formattedData = Object.entries(sectorData).map(
    ([sector, intensity], i) => ({
      id: sector,
      label: sector,
      value: intensity,
  ...
  const totalIntensity = formattedData.reduce((sum, item) => sum + item.value, 0);
    ...
    >
      ...
      >
        <Typography variant="h6">
          {!isDashboard && "Total Intensity:"} {totalIntensity.toFixed(2)}
        </Typography>
      </Box>
    </Box>
  );
